...
export default function RepositoryMap({features}:{features:Feature[]}) {
  ...
  return <>{features.map((f,i)=><article className="feature" key={f.id}><div className="featureHead"><b>{String(i+1).padStart(2,'0')}</b><h3>{f.name}</h3></div><ol>{f.flow_steps.map((s,j)=>{
    ...
    return <li key={`${s.symbol_id}-${j}`} className={`flowStep flowStep--${certainty}`} data-flow-certainty={certainty}>
      <span>{s.symbol_name??s.symbol_id}</span>{s.relation&&<small>{s.relation}</small>}
