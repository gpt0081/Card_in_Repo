...
import { ConceptCandidate, buildConceptCandidates } from './concepts';
import './app.css';

const terminal = new Set(['READY','FAILED_TERMINAL','FAILED_EXHAUSTED']);
type View = 'features'|'files'|'concepts';
export default function App() {
  ...
  const [features,setFeatures]=useState<Feature[]>([]); const [files,setFiles]=useState<RepositoryFile[]>([]); const [concepts,setConcepts]=useState<ConceptCandidate[]>([]);
  const [view,setView]=useState<View>('features'); const [error,setError]=useState<string>();
  async function submit(e:FormEvent){e.preventDefault();setError(undefined);setFeatures([]);setFiles([]);setConcepts([]);setView('features');try{const r=await submitRepository(url.trim());setId(r.id);setState(r.state)}catch(e){setError(e instanceof Error?e.message:String(e))}}
  ...
  async function openConcepts(){if(view==='features')return;try{const loaded=await loadFiles();if(!loaded)return;setConcepts(buildConceptCandidates(features,loaded));setView('concepts')}catch(e){setError(e instanceof Error?e.message:String(e))}}
  ...
  return <main><header><span className="eyebrow">CARD IN REPO</span><h1>Read the flow before the files.</h1><p>Paste a public GitHub repository. The first result is its feature and execution map, not a wall of code cards.</p></header>
    ...
    {state==='READY'&&view==='concepts'&&<section><div className="sectionTitle"><span>03</span><h2>Concepts</h2></div><p>Concept candidates are assembled from verified execution flow and symbol locations. Teaching text comes later.</p>{concepts.length===0?<p>No evidence-backed concepts were discovered.</p>:concepts.map(concept=><article className="feature concept" key={concept.id}><div className="featureHead"><b>FLOW</b><h3>{concept.name}</h3></div><ol>{concept.evidence.map(item=><li key={`${concept.id}-${item.symbol_id}-${item.order}`}><span>{item.symbol_name}</span><small>{item.path??'path unavailable'}{item.relation?` · ${item.relation}`:''}</small></li>)}</ol></article>)}</section>}
    <nav aria-label="Learning views"><button className={view==='features'?'active':''} onClick={()=>setView('features')}>Feature map</button><button className={view==='files'?'active':''} disabled={state!=='READY'} onClick={openFiles}>Files</button><button className={view==='concepts'?'active':''} disabled={state!=='READY'||view==='features'} onClick={openConcepts}>Concepts</button><button disabled>Cards</button></nav>
  </main>
}
