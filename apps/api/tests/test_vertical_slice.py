...
def test_public_github_analysis_uses_repository_snapshot_and_cross_file_flow(monkeypatch):
    ...
    monkeypatch.setattr(app_module, "resolve_github_repository", lambda repository_url, ref=None: GitHubRepositorySnapshot(repository="octo/demo", commit_sha=sha, files=files))
    ...
    features = client.get(f"/v1/analyses/{payload['id']}/features").json()["features"]
    entry = next(feature for feature in features if feature["name"] == "entry")
