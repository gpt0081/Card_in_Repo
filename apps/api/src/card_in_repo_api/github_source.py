...
def resolve_github_repository(repository_url: str, ref: str | None = None, *, max_files: int = 200, max_total_bytes: int = 2_000_000) -> GitHubRepositorySnapshot:
    """Fetch a bounded immutable snapshot of Python source files from a public repo."""
    ...
    entries = [item for item in tree.get("tree", []) if item.get("type") == "blob" and str(item.get("path", "")).endswith(".py")]
    entries.sort(key=lambda item: item["path"])
    if not entries:
        raise GitHubSourceError("repository contains no supported Python files")
    if len(entries) > max_files:
        raise GitHubSourceError(f"repository exceeds Python file limit ({max_files})")
    declared_size = sum(int(item.get("size") or 0) for item in entries)
    if declared_size > max_total_bytes:
        raise GitHubSourceError(f"repository exceeds supported Python source size ({max_total_bytes} bytes)")
    files: dict[str, str] = {}
    actual_size = 0
    for item in entries:
        path = item["path"]
